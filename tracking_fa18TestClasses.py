...
class GameScoreTest(testClasses.TestCase):
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        handle.write(
            "# You must win at least %d/10 games with at least %d points"
            % (self.numWinsForCredit, self.min_score)
        )
        handle.close()
...
class ZeroWeightTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if pac.getReset():
            grades.addMessage(
                "%s) successfully handled all weights = 0" % grades.currentQuestion
            )
            return self.testPass(grades)
        else:
            grades.addMessage(
                "%s) error handling all weights = 0" % grades.currentQuestion
            )
            return self.testFail(grades)
...
class OutputTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        ...
        self.preamble = compile(
            testDict.get("preamble", ""), "%s.preamble" % self.getPath(), "exec"
        )
...
class InferenceAgent(bustersAgents.BustersAgent):
    ...
    def __init__(
        self, inference, ghostAgents, seed, elapse=True, observe=True, burnIn=0
    ):
...
class ZeroWeightAgent(bustersAgents.BustersAgent):
    ...
    def __init__(
        self, inference, ghostAgents, grades, seed, disp, elapse=True, observe=True
    ):
        self.inferenceModules = [inference(a) for a in ghostAgents]
        self.elapse = elapse
        self.observe = observe
    ...
    def distCompare(self, dist, refDist):
        ...
        if l2 > self.L2Tolerance:
            if self.errors == 0:
                ...
                summary = (
                    "%s) Distribution deviated at move %d by %0.4f (squared norm) from the correct answer.\n"
                    % t
                )
